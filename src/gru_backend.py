"""GRU-based creative recipe generator.

Bridges the separately-trained GRU seq2seq title model + RAG composer from
the sibling demo project into this app. The flow is:

    1. User ingredients → normalize_user_text()
    2. GRU seq2seq → predicted recipe title ("Chicken Something")
...
DEFAULT_CKPT = os.environ.get(
    "RR_GRU_CKPT",
    str(Path(__file__).resolve().parent.parent / "gru_checkpoints" / "best.pt"),
)
DISABLED = os.environ.get("RR_NO_GRU", "0") == "1"


@dataclass
class GRURecipeResult:
    ok: bool
    text: str = ""                  # rendered markdown on success, error message on failure
    title: str = ""                 # GRU-predicted title
    model: str = "seq2seq-gru"
...
def _load_bundle():
    ...
    if DISABLED:
        _load_error = "GRU disabled via RR_NO_GRU=1."
        return None
    ...
    with _bundle_lock:
        ...
        if not ckpt.exists():
            _load_error = f"GRU checkpoint not found at {ckpt}."
            return None
        try:
            # Deferred import so starting the app without torch doesn't crash.
            from .gru.predict import load_checkpoint
            print(f"[gru] Loading checkpoint {ckpt} ...", flush=True)
            _bundle = load_checkpoint(str(ckpt))
            _, _, _, dev = _bundle
            print(f"[gru] Loaded on {dev}.", flush=True)
        except Exception as e:  # pragma: no cover — defensive
            _load_error = f"Failed to load GRU: {e}"
            print(f"[gru] {_load_error}", flush=True)
            return None
    return _bundle


def check_gru() -> tuple[bool, str]:
    """Non-blocking health check for the UI."""
    if DISABLED:
        return False, "GRU disabled (RR_NO_GRU=1)"
    ckpt = Path(DEFAULT_CKPT)
    if not ckpt.exists():
        return False, f"GRU checkpoint missing at {ckpt}"
    try:
        import torch  # noqa: F401
    except ImportError:
        return False, "torch not installed"
    try:
        from .gru.allrecipes import check_allrecipes
...
def _render_markdown(draft: dict) -> str:
    ...
    title = _clean_line(draft.get("title")) or "GRU Recipe"
...
def _build_matches_from_allrecipes(
    ...
    *,
    ...
    try:
        from .gru.allrecipes import build_matches

        return build_matches(ingredients, predicted_title=predicted_title, top_k=5)
    except Exception as e:
        print(f"[gru] allrecipes grounding failed ({e})")
        return []


def generate_gru_recipe(
    ingredients: list[str],
    *,
    engine,
    pantry_items: Optional[list[str]] = None,
) -> GRURecipeResult:
    """Given a list of ingredient strings, return a GRU-drafted recipe.

    `engine` is accepted for backward compatibility with the UI but the GRU
    ...
    """
    if not ingredients:
        return GRURecipeResult(ok=False, text="No ingredients — type something first.")

    bundle = _load_bundle()
    if bundle is None:
        return GRURecipeResult(ok=False, text=f"⚠️ GRU unavailable: {_load_error}")

    from .gru.predict import greedy
    from .gru.rag import generate_recipe_draft
    ...
    try:
        ...
        return GRURecipeResult(ok=False, text=f"⚠️ GRU inference failed: {e}")
    ...
    try:
        ...
        return GRURecipeResult(
            ok=False, text=f"⚠️ GRU draft composition failed: {e}",
            title=predicted_title,
        )

    md = _render_markdown(draft)
    return GRURecipeResult(ok=True, text=md, title=str(draft.get("title") or predicted_title))
